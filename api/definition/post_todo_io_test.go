...
func TestCreatePostTodoRequestBody_正常系(t *testing.T) {
	e := TestInit()
	var data = `{
		"title": "Test Title",
		"body": "Test Body"
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/todos", strings.NewReader(data))
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)

	result, err := CreatePostTodoRequestBody(c)
	assert.NoError(t, err)
	assert.IsType(t, &PostTodoRequestBody{}, result)
}

func TestCreatePostTodoRequestBody_required(t *testing.T) {
	e := TestInit()
	...
	for element, value := range data {
		...
		case "title":
			for _, v := range value {
				req := httptest.NewRequest(http.MethodPost, "/api/v1//todos", strings.NewReader(v))
				req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
				rec := httptest.NewRecorder()
				c := e.NewContext(req, rec)

				_, err := CreatePostTodoRequestBody(c)
...
func TestCreatePostTodoRequestBody_lte(t *testing.T) {
	e := TestInit()
	var data = []string{
		// 256文字（異常系）
		`{
	...
	for index, value := range data {
		if index != 0 {
			req := httptest.NewRequest(http.MethodPost, "/api/v1/todos", strings.NewReader(value))
			req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
			rec := httptest.NewRecorder()
			c := e.NewContext(req, rec)
